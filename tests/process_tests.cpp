...
#include <cs/process/process.hpp>
...
static bool isProgramExists() {
    static bool status = ::isFileExists(programPath);
    return status;
}
...
TEST(Process, HighLoadUsage) {
    ...
    cs::Connector::connect(&process.started, [&]{
        ...
        cslog() << "Launch count " << launchCount.load(std::memory_order_acquire);
    });

    cs::Connector::connect(&process.finished, [&](int, const std::system_error&) {
        finishCount.fetch_add(1u, std::memory_order_release);
        cslog() << "Finish count " << finishCount.load(std::memory_order_acquire);
